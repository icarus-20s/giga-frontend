...
const SECTIONS = [
    {
        ...
        description:
            "Offers robust accounting tools for compliance, reporting, and resource management.",
        image: work,
        modules: [
            {
    ...
    },
    {
        ...
        description:
            "Complete HR management system with leave, attendance, payroll, and employee lifecycle automation.",
        image: work,
        modules: [
            {
                name: "Leave Module",
...
const ModuleBadge = ({ index }) => {
    ...
    return (
        <span
            className={`px-3 py-1.5 text-xs font-semibold uppercase tracking-wider rounded-full border ${badge.text} ${badge.background} ${badge.border}`}
        >
...
const SectionImage = ({ section, index }) => {
    ...
    return (
        <div className={isReversedLayout(index) ? "lg:order-1" : ""}>
            <FadeUp>
                <div
                    className={`rounded-2xl overflow-hidden border ${card.border} ${card.background}`}
                >
                    <img
                        src={section.image}
                        alt={section.title}
...
const FeatureItem = ({ feature }) => {
    ...
    return (
        <div
            className={`flex gap-4 items-start ${THEME.spacing.feature} rounded-lg ${card.background} border ${card.border}`}
        >
            <div className="flex-shrink-0 mt-1">
                <div
                    className={`w-5 h-5 rounded-full ${accent.secondary} flex items-center justify-center`}
                >
                    <CheckIcon />
                </div>
            </div>
...
const ModuleHeader = ({ module, index }) => {
    ...
    return (
        <div
            className={`${THEME.spacing.cardHeader} ${card.background} border-b ${card.border}`}
        >
...
const ModuleCard = ({ module, index }) => {
    ...
    return (
        <article
            className={`rounded-2xl border ${card.border} ${card.background} ${card.shadow}`}
        >
            <FadeUp>
                <ModuleHeader module={module} index={index} />
                <div
                    className={`${THEME.spacing.cardBody} grid md:grid-cols-2 gap-6`}
                >
...
const SectionContent = ({ section }) =>
    section.modules ? <ModulesList modules={section.modules} /> : null;
...
const Section = ({ section, index }) => {
    ...
    return (
        <section data-id={section.id}>
            <div
                className={`grid lg:grid-cols-2 ${sectionGap} items-center mb-16`}
            >
    ...
    return (
        <div className="min-h-screen">
            <main className={background}>
                <div
                    className={`w-full mx-auto px-6 lg:px-8 ${sectionPadding}`}
                >
