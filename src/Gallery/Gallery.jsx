...
const DUMMY_PHOTOS = [
  ...
  "https://picsum.photos/400/720?random=8",
];
...
const Gallery = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setPhotos(DUMMY_PHOTOS.sort(() => Math.random() - 0.5));
      setLoading(false);
    }, 1000);
    return () => clearTimeout(timer);
  }, []);

  const openModal = useCallback((index) => { setCurrentIndex(index); setModalOpen(true); }, []);
  const closeModal = useCallback(() => setModalOpen(false), []);
  const prevPhoto = useCallback(() => setCurrentIndex(prev => (prev === 0 ? photos.length - 1 : prev - 1)), [photos.length]);
  const nextPhoto = useCallback(() => setCurrentIndex(prev => (prev === photos.length - 1 ? 0 : prev + 1)), [photos.length]);
